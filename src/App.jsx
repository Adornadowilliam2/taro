...
function App() {
  ...
  const shuffleArray = (arr) => {
    const newArr = [...arr];
    for (let i = newArr.length - 1; i > 0; i--) { // Fix: Use i-- instead of i++
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setShuffle((current) => shuffleArray(current)); // Shuffle the array
      AssassinMode((current) =>
        current === "https://github.com/Adornadowilliam2/taro/blob/main/src/assets/fat.png?raw=true"
          ? "https://github.com/Adornadowilliam2/taro/blob/main/src/assets/assassinmode.png?raw=true"
          : "https://github.com/Adornadowilliam2/taro/blob/main/src/assets/fat.png?raw=true"
      );
    }, 6000); // Shuffle and toggle every 6 seconds
  ...
  return (
    <>
      <nav className='p-2 flex items-center bg-gray-600 text-white gap-2'>
        <img src={fat} alt="sakamoto logo" className='w-20' />
        Sakamoto Content
      </nav>
      <main className='p-5 flex items-center flex-wrap justify-evenly gap-5'>
        {shuffle.map((item) => (
          <div key={item.id} className='img'>
            <img src={item.image} alt={`sakamoto ${item.id}`} />
          </div>
        ))}
      </main>
    </>
